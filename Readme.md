...
## Usage
### data preprocessing
In order to run scDIFF, we need to first create an anndata object from the raw data. The .h5ad file should have cells as obs and peaks as var. The var must include at least three columns: chr, start, and end, which indicate the genomic region of each peak. Additionally, the obs should contain two columns: Batch and CellType (reference data), where Batch distinguishes between reference and query data, and CellType indicates the true label of the cell. We provide methods for processing from raw datasets to h5ad format in `./dataPreprocessing/create_h5ad.ipynb` and `./dataPreprocessing/create_pairs.ipynb`. After processing the generated .h5ad format data, you should place them in the file directory as shown in the directory structure.
...
https://www.synapse.org/Synapse:syn52570477 for GRCh38.primary_assembly.genome.fa.h5.
https://www.synapse.org/Synapse:syn52566798 for mm9.fa.h5.
https://www.synapse.org/Synapse:syn52567001 for mm10.fa.h5.

To obtain tissue-specific histone modification information, histone ChIP-seq peak files (in BED format) are aligned to the genome using LiftOver, with '1' indicating the presence and '0' indicating the absence of modification signals for each corresponding protein. In cases where multi-source reference sets derive from different tissues, we consider two strategies for handling tissue-specific histone modifications: Union strategy: if a nucleotide locates within any tissue's ChIP-seq peak, it is marked as '1', otherwise marked as '0'; Intersection strategy: if a nucleotide locates within all tissues' ChIP-seq peaks, it is marked as '1', otherwise marked as '0'. All ChIP-seq peaks are directly downloaded from the ensemble database.These processed histone modification signals are stored in .bigWig format. We provide methods for processing from raw datasets to bigwig format in `./dataPreprocessing/intersect_union.sh`, `./dataPreprocessing/mm10bed_to_hg19bed.sh`, `./dataPreprocessing/mm10bed_to_mm9bigwig.sh`, `./dataPreprocessing/hg38bed_to_hg38bigwig.sh` and `./dataPreprocessing/mm10bed_to_mm10bigwig.sh`. After processing the bed files with the above script to generate the corresponding bigwig files, you should rename them to H3k4me1_mm9.bigWig, H3k4me3_mm9.bigWig, H3k27ac_mm9.bigWig, and place them in the file directory as shown in the directory structure.
