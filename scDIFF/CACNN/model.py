...
class CACNN(nn.Module):
    ...
    def __init__(self, n_cells: int, batch_ids: Optional[Iterable[int]] = None, use_reg_cell=False, hidden_size=32, seq_len: int = 1344, epifeature_dim: int = 3):
        ...
        self.epifeature_dim = epifeature_dim  # Dynamically adjust the number of channels

        # 1
        input_channels = 4 + epifeature_dim  # The number of input channels is 4 (one-hot) + epifeature_dim
        current_len = seq_len
    ...
    def forward(self, sequence: Tensor, epifeature: Tensor) -> Tensor:
        ...
        epifeature: (batch_size, epifeature_dim, seq_len ) 

        """
        # transform sequence to one-hot vector
        sequence = self.onehot[sequence.long()].transpose(1, 2)  # (batch_size, 4, seq_len)

        # Concatenate sequence and epifeature
        x = torch.cat((sequence, epifeature), dim=1)  # shape: (batch_size, 4 + epifeature_dim, seq_len)
