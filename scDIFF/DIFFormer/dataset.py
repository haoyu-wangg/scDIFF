...
def bbknn_construct_graph(datastr_name, data, batch_info, edge_ratio, train_label, test_label, save_path):
    """
        adata_train: remove unknown->remove rare and intersection->smote
        adata_test: remove unkown->remove rare and intersection
        datastr_name: unused
        data: np.vstack((adata_train.X, adata_test.X))
        batch_info: list like [100, 200]
        edge_ratio: edge_ratio->ratio->prune
        check:
            train_label: adata_train.obs[label_name].values
            test_label: adata_test.obs[label_name].values
            save: 
...
def atac_data_graph_construction(data_path, train_name_list, test_name, sample_ratio, edge_ratio, label_name, save_path, save_unknown, save_rare, no_smote):
    ...
    if not save_unknown:
        ...
        adata_train = adata_train[adata_train.obs[label_name] != 'Unknown']
        adata_train = adata_train[adata_train.obs[label_name] != 'unknown']
        #adata_train = adata_train[adata_train.obs[label_name] != 'NA']
        #adata_train = adata_train[adata_train.obs[label_name].values.notnull()]

        adata_test = adata_test[adata_test.obs[label_name] != 'Unknown']
        adata_test = adata_test[adata_test.obs[label_name] != 'unknown']
        #adata_test = adata_test[adata_test.obs[label_name] != 'NA']
        #adata_test = adata_test[adata_test.obs[label_name].values.notnull()]
