...
def get_embedding(model, dataset, split_idx, args, adata, le, save_path):
    model.eval()
    pred, layer_ = model(dataset.graph['node_feat'], dataset.graph['edge_index'], return_embedding=True)
...
def get_embedding_weight(model, dataset, split_idx, args, adata, le, save_path, train_shape, test_shape):
    model.eval()
    pred, layer_, weight_ = model(dataset.graph['node_feat'], dataset.graph['edge_index'], train_shape, test_shape, return_embedding=True)
