...
patience = args.patience  # Stop early
patience_counter = 0      # Patience counter, initialized to 0
...
for run in range(args.runs):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if epoch % args.eval_step == 0:
            ...
            if result[3] < best_val:  # If there is an improvement in the validation set metrics
                best_val = result[3]
...
# 2025.2.17 Consider whether to cancel the early stop
            # Check if it stopped early
            if patience_counter >= patience:
                print(f"Early stopping at epoch {epoch} in run {run}")
                break  # Terminate the training cycle
