    ...
    def __getitem__(self, index):
        ...
        for bw_file, bw in zip(self.bw_list, self.bwls):
            try:
                ...
                # Apply specific threshold based on BigWig file name
                # threshold = 2.0  # 默认阈值
                # for keyword, thres in thresholds.items():
                #     if keyword in bw_file:
                #         threshold = thres
                #         break  # 找到匹配的阈值后退出循环
                # # 是否截断阈值
                # feature = (feature > threshold).astype(np.float32)
