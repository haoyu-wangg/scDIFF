...
def test_model(model, loader):
    ...
    for it, (seq, epi, target) in enumerate(tqdm(loader)):
        seq = seq.to(device)
        epi = epi.to(device)
        output, _ = model(seq, epi)
        output = output.detach()

        if torch.isnan(output).any():
            print(f"NaN detected in output at batch {it}")

        output = torch.sigmoid(output).cpu().numpy()
        target = target.numpy().astype(np.int8)

        if np.isnan(target).any():
            print(f"NaN detected in target at batch {it}")
    ...
    if np.isnan(all_pred).any():
        print("NaN detected in all_pred after concatenation")
    if np.isnan(all_label).any():
        print("NaN detected in all_label after concatenation")
    ...
    for i in tqdm(test_inds, desc="Calculating AP"):
        if np.isnan(all_label[i]).any() or np.isnan(all_pred[i]).any():
            print(f"NaN detected at index {i}")
            continue
        try:
            score = roc_auc_score(all_label[i], all_pred[i])
...
def get_args():
    ...
    p.add_argument('-i', "--data", type=str, default="/home/daozhang/ChIP-SANGO/preprocessed_data/BoneMarrowB_BoneMarrowA.h5ad", help="path to the input h5ad file")
    ...
    p.add_argument("--num-workers", type=int, default=12, help="num of workers")
    ...
    p.add_argument("--epifeature_dim", type=int, default=3, help="dim of epifeature")
    p.add_argument("--root_path", default="/home/daozhang/ChIP-SANGO/data/", help="epidata root path")
    p.add_argument("--tissue", default="BoneMarrow", help="tissue name")
    p.add_argument("--bw_list", nargs='+', default=["H3k4me1_mm9.bigWig", "H3k27ac_mm9.bigWig", "H3k4me3_mm9.bigWig"], 
                   help="list of bigWig files")
...
if __name__ == "__main__":
    ...
    bw_path = os.path.join(args.root_path, args.tissue)
    bw_list= args.bw_list # ["H3k4me1_mm9.bigWig", "H3k27ac_mm9.bigWig","H3k4me3_mm9.bigWig"]
    # load dataset

    # ds = dataset.SingleCellDataset(dataset.load_adata(args.data), seq_len=args.seq_len, genome=genome)
    ds = dataset.SingleCellDataset(dataset.load_adata(args.data), seq_len=args.seq_len, genome=genome, bw_path=bw_path, bw_list=bw_list)
    ...
    # make model
    model = CACNN(n_cells=ds.X.shape[1], hidden_size=args.z, seq_len=args.seq_len, use_reg_cell=args.use_reg_cell, epifeature_dim=args.epifeature_dim).to(device)
    ...
    if not args.w:
        ...
        for epoch in range(max_epoch):
            ...
            model.train()
            for it, (seq, epi, target) in enumerate(pbar):
                seq = seq.to(device)
                epi = epi.to(device)
                target = target.to(device)
                optimizer.zero_grad()
                with autocast("cuda"):
                    output, lr_reg_cell = model(seq, epi)
