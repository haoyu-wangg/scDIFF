...
INPUT_DIR=$1
OUTPUT_DIR=$2
GENOME_SIZE="$(dirname "$0")/mm10.chrom.sizes"  # Use mm10 chromosome size file from script directory
...
for file in "$INPUT_DIR"/*.bed.gz; do
    ...
    # 4. Handle overlapping regions (use bedtools merge and calculate maximum values)
